...
{
    Uint32 maxArmatures;
    Skeleton3D* armatureGFC_List;
}ArmatureManager3D;

static ArmatureManager3D armature_manager3d = { 0 };

void gf3d_armature_delete(Skeleton3D* armature);//permanently clean up
void gf3d_armature_parse_poses(Skeleton3D* armature, GLTF* gltf);
...
Bone3D* gf3d_armature_get_bone_by_id(Skeleton3D* armature, Uint32 id);

int gf3d_armature_get_bone_index(Skeleton3D* armature, Uint32 boneId)
{
    Bone3D* bone;
...
Skeleton3D* gf3d_armature_load(const char* filename)
{
    GLTF* gltf;
    Skeleton3D* armature;
...
GFC_Matrix4* gf3d_armature_get_bone_matrix_by_name(Skeleton3D* armature, const char* name)
{
    Bone3D* bone;
...
GFC_Matrix4* gf3d_armature_get_bone_pose_matrix_by_name(Skeleton3D* armature, Uint32 frame, const char* name)
{
    Bone3D* bone;
    BonePose3D* pose;
    bone = gf3d_armature_get_bone_by_name(armature, name);
...
GFC_Matrix4* gf3d_armature_get_pose_matrices(Skeleton3D* armature, Uint32 frame, Uint32* boneCount)
{
    /*Pose3D* pose;
    if (!armature || !armature->poses)
    {
        slog("null armature");
        return NULL;
    }
    ...
    return pose->bones;*/
    if (!armature || !armature->poses) {
        slog("Error: Armature or poses is NULL.");
        return NULL;
    }
    else {
        slog("Armature is valid");
    }

    Pose3D* pose = gfc_list_get_nth(armature->poses, frame);
    if (!pose) {
        slog("Error: Pose for frame %d is NULL.", frame);
        return NULL;
    }
    else {
        slog("Pose list is not Null");
    }

    if (boneCount) *boneCount = pose->boneCount;
    slog("Requested bones exist");
    return pose->bones;
}
...
Bone3D* gf3d_armature_get_bone_by_name(Skeleton3D* armature, const char* name)
{
    int i, c;
    Bone3D* bone;
...
Bone3D* gf3d_armature_get_root(Skeleton3D* arm)
{
    int i, c;
    Bone3D* bone = NULL;
    if (!arm)return NULL;
...
void gf3d_armature_calc_bone_mats(Skeleton3D* arm)
{
    if (!arm)return;
...
void gf3d_armature_calc_joint_matrices(Skeleton3D* armature)
{
    Bone3D* bone;
    int i, c;
    if (!armature)return;
...
void gf3d_armature_calculate_poses(Skeleton3D* armature)
{
    int i, c, j;
    Bone3D* bone;
    BonePose3D* bonePose;
...
void gf3d_armature_parse_joints(Skeleton3D* armature, SJson* nodes, SJson* joints)
{
    Bone3D* bone;
    SJson* node;
...
void gf3d_armature_parse_build_poses(Skeleton3D* armature)
{
    // Sanity check
    if (!armature || armature->maxFrames == 0) {
        slog("Error: Armature not initialized or has no frames.");
        return;
    }
    ...
    slog("Armature maxFrames: %d", maxFrames); // Log the number of frames calculated
    if (maxFrames <= 1)
    {
...
void gf3d_armature_parse_animation(Skeleton3D* armature, GLTF* gltf, SJson* animation)
{
    int bufferIndex;
    int itemCount;
    int i, c;
...
void gf3d_armature_parse_inverseBindMatrices(Skeleton3D* armature, GLTF* gltf, SJson* skin)
{
    int valuesAccessor;
    int bufferIndex;
...
Skeleton3D* gf3d_armature_parse(GLTF* gltf)
{
    int i, c;
    SJson* array, * item;
    Skeleton3D* armature = gf3d_armature_new();
    if (!gltf)return NULL;
...
void gf3d_armature_draw_bone_poses(Skeleton3D* arm, Uint32 frame)
{
    Bone3D* bone;
    BonePose3D* pose;
    int i, c;
...
void gf3d_armature_draw_bones(Skeleton3D* arm)
{
    Bone3D* bone;
    int i, c;
    if ((!arm) || (!arm->bones))return;
...
void gf3d_armature_system_init(Uint32 maxArmatures)
{
    ...
    armature_manager3d.armatureGFC_List = (Skeleton3D*)malloc(sizeof(Skeleton3D) * maxArmatures);
    ...
    memset(armature_manager3d.armatureGFC_List, 0, sizeof(Skeleton3D) * maxArmatures);
...
Skeleton3D* gf3d_armature_new()
{
    ...
    for (i = 0; i < armature_manager3d.maxArmatures; i++)
    {
        ...
        {
            memset(&armature_manager3d.armatureGFC_List[i], 0, sizeof(Skeleton3D));
...
void gf3d_armature_delete(Skeleton3D* armature)
{
    int i, c;
    if (!armature)return;
    ...
    memset(armature, 0, sizeof(Skeleton3D));
}

void gf3d_armature_free(Skeleton3D* armature)
{
    if (!armature) return;
    armature->refCount--;
}
...
SkeletonUBO gf3d_armature_get_ubo(
    Skeleton3D* armature,
    Uint32      frame)
{
    /*GFC_Matrix4* bones;
    Uint32 boneCount = 0;
    SkeletonUBO boneUbo = { 0 };
    ...
    slog("Armature and poses are valid");

    GFC_Matrix4* bones = gf3d_armature_get_pose_matrices(armature, frame, NULL);
    if (!bones) {
        slog("Error: Failed to retrieve bones for frame %d.", frame);
        return boneUbo;
    }
    slog("Pose matrices were obtained succesfully");

    memcpy(boneUbo.bones, bones, sizeof(GFC_Matrix4) * armature->bindCount);
    return boneUbo;
}
